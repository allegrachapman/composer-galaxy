---
name: Erik Satie
wikidata: Q187192
wikipedia: https://en.wikipedia.org/wiki/Erik_Satie
thumbnail: https://upload.wikimedia.org/wikipedia/commons/thumb/5/58/Ericsatie.jpg/250px-Ericsatie.jpg
birth: '1866'
birth_source: infobox
death: '1925'
death_source: infobox
nationality: Honfleur, France
nationality_source: infobox
era: 20th-century
era_source: llm
teachers:
- name: Gustave Vinot
  source: llm
  quote: he began taking music lessons with a local organist, Gustave Vinot
- name: Albert Lavignac
  source: llm
  quote: He studied solfeggio with Albert Lavignac
- name: Émile Decombes
  source: llm
  quote: and piano with Émile Decombes, who had been a pupil of Frédéric Chopin
- name: Georges Mathias
  source: llm
  quote: in the intermediate piano class of his stepmother's former teacher, Georges
    Mathias
- name: Albert Roussel
  source: llm
  quote: Satie studied counterpoint with Albert Roussel
  corroborated_by:
  - llm
  - wiki
- name: Vincent d'Indy
  source: llm
  quote: and composition with d'Indy
- name: Ernest Guiraud
  source: wiki
  source_url: https://en.wikipedia.org/wiki/List_of_music_students_by_teacher:_G_to_J
  corroborated_by:
  - llm
  - wiki
- name: Antoine Taudou
  source: wiki
  source_url: https://en.wikipedia.org/wiki/List_of_music_students_by_teacher:_T_to_Z
students:
- name: Francis Poulenc
  source: wiki
  source_url: https://en.wikipedia.org/wiki/List_of_music_students_by_teacher:_R_to_S
mentors: []
---





# Erik Satie

[Wikipedia](https://en.wikipedia.org/wiki/Erik_Satie)

## Notes

_Add your own notes here. This section is preserved across re-seeds._
