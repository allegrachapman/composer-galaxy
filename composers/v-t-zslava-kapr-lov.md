---
name: Vítězslava Kaprálová
wikidata: Q271624
wikipedia: https://en.wikipedia.org/wiki/Vítězslava_Kaprálová
thumbnail: https://upload.wikimedia.org/wikipedia/commons/thumb/8/8b/Kapralova1935.jpg/250px-Kapralova1935.jpg
birth: '1915'
birth_source: infobox
death: '1940'
death_source: infobox
nationality: Brno, Moravia, Austria-Hungary
nationality_source: infobox
era: 20th-century
era_source: llm
teachers:
- name: Vilém Petrželka
  source: wiki
- name: Zdeněk Chalabala
  source: llm
  quote: conducting with Zdeněk Chalabala at the Brno Conservatory
- name: Vítězslav Novák
  source: llm
  quote: continued her musical education with Vítězslav Novák (1935–37)
- name: Václav Talich
  source: llm
  quote: and Václav Talich (1935–36) in Prague
- name: Bohuslav Martinů
  source: llm
  quote: with Bohuslav Martinů, Charles Munch (1937–39)
- name: Charles Munch
  source: llm
  quote: with Bohuslav Martinů, Charles Munch (1937–39)
- name: Nadia Boulanger
  source: llm
  quote: according to some unverified accounts, with Nadia Boulanger (1940) in Paris
students: []
mentors: []
---





# Vítězslava Kaprálová

[Wikipedia](https://en.wikipedia.org/wiki/Vítězslava_Kaprálová)

## Notes

_Add your own notes here. This section is preserved across re-seeds._
