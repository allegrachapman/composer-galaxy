---
name: Isidor Philipp
wikidata: Q707752
wikipedia: https://en.wikipedia.org/wiki/Isidor_Philipp
thumbnail: https://upload.wikimedia.org/wikipedia/commons/thumb/6/60/Isidor_Philipp.jpg/250px-Isidor_Philipp.jpg
birth: '1863'
birth_source: infobox
death: '1958'
death_source: infobox
nationality: Budapest, Austrian Empire
nationality_source: infobox
era: 20th-century
era_source: llm
teachers:
- name: Georges Mathias
  source: wiki
- name: Camille Saint-Saëns
  source: llm
  quote: Other teachers included Camille Saint-Saëns
- name: Stephen Heller
  source: llm
  quote: Other teachers included Camille Saint-Saëns, Stephen Heller
- name: Théodore Ritter
  source: llm
  quote: and Théodore Ritter (a pupil of Franz Liszt)
students:
- name: Harold Bradley (pianist)
  source: wiki
- name: Guiomar Novaes
  source: wiki
- name: Aaron Copland
  source: llm
  quote: students who became notable... includes... Aaron Copland
- name: Albert Schweitzer
  source: llm
  quote: includes... noted philosopher Albert Schweitzer
- name: Igor Stravinsky
  source: llm
  quote: Philipp taught piano to Igor Stravinsky
- name: Soulima Stravinsky
  source: llm
  quote: Philipp taught piano to Igor Stravinsky and his son Soulima
- name: Beveridge Webster
  source: llm
  quote: students... includes... Beveridge Webster
- name: Emma Boynet
  source: wiki
  source_url: https://en.wikipedia.org/wiki/List_of_music_students_by_teacher:_N_to_Q
- name: John L. Buttrick
  source: wiki
  source_url: https://en.wikipedia.org/wiki/List_of_music_students_by_teacher:_N_to_Q
- name: Serge Conus
  source: wiki
  source_url: https://en.wikipedia.org/wiki/List_of_music_students_by_teacher:_N_to_Q
- name: Jeanne-Marie Darré
  source: wiki
  source_url: https://en.wikipedia.org/wiki/List_of_music_students_by_teacher:_N_to_Q
- name: Pierre Dervaux
  source: wiki
  source_url: https://en.wikipedia.org/wiki/List_of_music_students_by_teacher:_N_to_Q
- name: Ania Dorfmann
  source: wiki
  source_url: https://en.wikipedia.org/wiki/List_of_music_students_by_teacher:_N_to_Q
- name: Rolande Falcinelli
  source: wiki
  source_url: https://en.wikipedia.org/wiki/List_of_music_students_by_teacher:_N_to_Q
- name: Felix Fox
  source: wiki
  source_url: https://en.wikipedia.org/wiki/List_of_music_students_by_teacher:_N_to_Q
- name: Jean Françaix
  source: wiki
  source_url: https://en.wikipedia.org/wiki/List_of_music_students_by_teacher:_N_to_Q
- name: Norman Fraser
  source: wiki
  source_url: https://en.wikipedia.org/wiki/List_of_music_students_by_teacher:_N_to_Q
- name: Henri Gagnon
  source: wiki
  source_url: https://en.wikipedia.org/wiki/List_of_music_students_by_teacher:_N_to_Q
- name: Youra Guller
  source: wiki
  source_url: https://en.wikipedia.org/wiki/List_of_music_students_by_teacher:_N_to_Q
- name: Georges Hugon
  source: wiki
  source_url: https://en.wikipedia.org/wiki/List_of_music_students_by_teacher:_N_to_Q
- name: Carol Klein
  source: wiki
  source_url: https://en.wikipedia.org/wiki/List_of_music_students_by_teacher:_N_to_Q
- name: Yvonne Loriod
  source: wiki
  source_url: https://en.wikipedia.org/wiki/List_of_music_students_by_teacher:_N_to_Q
- name: Nikita Magaloff
  source: wiki
  source_url: https://en.wikipedia.org/wiki/List_of_music_students_by_teacher:_N_to_Q
- name: Colin McPhee
  source: wiki
  source_url: https://en.wikipedia.org/wiki/List_of_music_students_by_teacher:_N_to_Q
- name: Federico Mompou
  source: wiki
  source_url: https://en.wikipedia.org/wiki/List_of_music_students_by_teacher:_N_to_Q
- name: Léo-Pol Morin
  source: wiki
  source_url: https://en.wikipedia.org/wiki/List_of_music_students_by_teacher:_N_to_Q
- name: Wilfrid Pelletier
  source: wiki
  source_url: https://en.wikipedia.org/wiki/List_of_music_students_by_teacher:_N_to_Q
- name: Émile Poillot
  source: wiki
  source_url: https://en.wikipedia.org/wiki/List_of_music_students_by_teacher:_N_to_Q
- name: Harrison Potter
  source: wiki
  source_url: https://en.wikipedia.org/wiki/List_of_music_students_by_teacher:_N_to_Q
- name: Phyllis Sellick
  source: wiki
  source_url: https://en.wikipedia.org/wiki/List_of_music_students_by_teacher:_N_to_Q
- name: Louise Talma
  source: wiki
  source_url: https://en.wikipedia.org/wiki/List_of_music_students_by_teacher:_N_to_Q
- name: Alexander Tcherepnin
  source: wiki
  source_url: https://en.wikipedia.org/wiki/List_of_music_students_by_teacher:_N_to_Q
- name: Paul Velucci
  source: wiki
  source_url: https://en.wikipedia.org/wiki/List_of_music_students_by_teacher:_N_to_Q
- name: Victor Young
  source: wiki
  source_url: https://en.wikipedia.org/wiki/List_of_music_students_by_teacher:_N_to_Q
- name: Frederick Jacobi
  source: wiki
mentors: []
---






# Isidor Philipp

[Wikipedia](https://en.wikipedia.org/wiki/Isidor_Philipp)

## Notes

_Add your own notes here. This section is preserved across re-seeds._
