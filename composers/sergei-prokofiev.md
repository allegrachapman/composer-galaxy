---
name: Sergei Prokofiev
wikidata: Q49481
wikipedia: https://en.wikipedia.org/wiki/Sergei_Prokofiev
thumbnail: https://upload.wikimedia.org/wikipedia/commons/thumb/0/03/Sergei_Prokofiev_circa_1918_over_Chair_Bain.jpg/250px-Sergei_Prokofiev_circa_1918_over_Chair_Bain.jpg
birth: '1891'
birth_source: infobox
death: '1953'
death_source: infobox
nationality: Sontsovka, Russian Empire
nationality_source: infobox
era: 20th-century
era_source: llm
teachers:
- name: Reinhold Glière
  source: llm
  quote: Prokofiev's first composition teacher
- name: Alexander Winkler
  source: llm
  quote: studied under, among others, Alexander Winkler for piano
- name: Anatoly Lyadov
  source: llm
  quote: studied under, among others, Anatoly Lyadov for harmony and counterpoint
- name: Nikolai Tcherepnin
  source: llm
  quote: studied under Nikolai Tcherepnin for conducting
- name: Nikolai Rimsky-Korsakov
  source: llm
  quote: Nikolai Rimsky-Korsakov for orchestration
- name: Anna Yesipova
  source: llm
  quote: studying piano under Anna Yesipova
- name: Alexander Glazunov
  source: wiki
  source_url: https://en.wikipedia.org/wiki/List_of_music_students_by_teacher:_G_to_J
- name: Yuri Pomerantsiev
  source: wiki
  source_url: https://en.wikipedia.org/wiki/List_of_music_students_by_teacher:_N_to_Q
- name: Serge Tcherepnin
  source: wiki
  source_url: https://en.wikipedia.org/wiki/List_of_music_students_by_teacher:_T_to_Z
- name: Jāzeps Vītols
  source: wiki
  source_url: https://en.wikipedia.org/wiki/List_of_music_students_by_teacher:_T_to_Z
students:
- name: Sviatoslav Richter
  source: llm
  quote: he wrote his Ninth Piano Sonata for the former
- name: Mstislav Rostropovich
  source: llm
  quote: his Symphony-Concerto for the latter
mentors: []
---





# Sergei Prokofiev

[Wikipedia](https://en.wikipedia.org/wiki/Sergei_Prokofiev)

## Notes

_Add your own notes here. This section is preserved across re-seeds._
