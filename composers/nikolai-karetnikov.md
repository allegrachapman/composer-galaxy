---
name: Nikolai Karetnikov
wikipedia: https://en.wikipedia.org/wiki/Nikolai_Karetnikov
thumbnail: null
birth: '1930'
birth_source: llm
death: '1994'
death_source: llm
nationality: Russian
nationality_source: llm
era: 20th-century
era_source: llm
teachers:
- name: Philip Herschkowitz
  source: wiki
- name: Alban Berg
  source: wiki
- name: Anton Webern
  source: wiki
- name: Vissarion Shebalin
  source: llm
  quote: his teachers were Vissarion Shebalin (composition)
- name: Tatiana Nikolayeva
  source: llm
  quote: Tatiana Nikolayeva (piano)
- name: Igor Sposobin
  source: llm
  quote: Igor Sposobin and Viktor Tsukkerman (theory)
- name: Viktor Tsukkerman
  source: llm
  quote: Igor Sposobin and Viktor Tsukkerman (theory)
students:
- name: Philip Herschkowitz
  source: wiki
- name: Anton Webern
  source: wiki
influenced_by:
- name: Alban Berg
  source: llm
  quote: a pupil of Berg and Webern
- name: Anton Webern
  source: llm
  quote: a pupil of Berg and Webern
influenced: []
---

# Nikolai Karetnikov

[Wikipedia](https://en.wikipedia.org/wiki/Nikolai_Karetnikov)

## Notes

_Add your own notes here. This section is preserved across re-seeds._
