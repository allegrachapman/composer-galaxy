---
name: William Bolcom
wikidata: Q710714
wikipedia: https://en.wikipedia.org/wiki/William_Bolcom
thumbnail: null
birth: '1938'
birth_source: infobox
death: null
death_source: null
nationality: American
nationality_source: infobox
era: 20th-century
era_source: llm
teachers:
- name: Darius Milhaud
  source: wiki
- name: Leonard Slatkin
  source: wiki
- name: George Frederick McKay
  source: llm
  quote: studied under faculty at University of Washington, including composition
    with professors George Frederick McKay
- name: John Verrall
  source: llm
  quote: composition with professors George Frederick McKay and John Verrall
- name: Berthe Poncy Jacobson
  source: llm
  quote: piano with Berthe Poncy Jacobson
- name: Leland Smith
  source: llm
  quote: studied with Leland Smith at Stanford University
- name: Olivier Messiaen
  source: llm
  quote: studied with Olivier Messiaen at the Paris Conservatoire
- name: George Rochberg
  source: wiki
  source_url: https://en.wikipedia.org/wiki/List_of_music_students_by_teacher:_R_to_S
students:
- name: Derek Bermel
  source: wiki
- name: Gabriela Lena Frank
  source: wiki
- name: David T. Little
  source: llm
  quote: Notable students include Derek Bermel, Gabriela Lena Frank, David T. Little
- name: Carter Pann
  source: llm
  quote: Notable students include Derek Bermel, Gabriela Lena Frank, David T. Little,
    Carter Pann
- name: Joel Puckett
  source: llm
  quote: Carter Pann, Joel Puckett, and Elena Ruehr
- name: Elena Ruehr
  source: llm
  quote: Carter Pann, Joel Puckett, and Elena Ruehr
- name: Andrew Bishop
  source: wiki
  source_url: https://en.wikipedia.org/wiki/List_of_music_students_by_teacher:_A_to_B
- name: Kenneth Hesketh
  source: wiki
  source_url: https://en.wikipedia.org/wiki/List_of_music_students_by_teacher:_A_to_B
- name: Chihchun Chi-sun Lee
  source: wiki
  source_url: https://en.wikipedia.org/wiki/List_of_music_students_by_teacher:_A_to_B
- name: Felicia Sandler
  source: wiki
  source_url: https://en.wikipedia.org/wiki/List_of_music_students_by_teacher:_A_to_B
- name: Matthew Tommasini
  source: wiki
  source_url: https://en.wikipedia.org/wiki/List_of_music_students_by_teacher:_A_to_B
mentors: []
---





# William Bolcom

[Wikipedia](https://en.wikipedia.org/wiki/William_Bolcom)

## Notes

_Add your own notes here. This section is preserved across re-seeds._
